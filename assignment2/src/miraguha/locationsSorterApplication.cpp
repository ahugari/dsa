...
#include "locationsSorter.h"

void qsort_array(int a[], int low, int high);
...
int main() {
    ...
    int arr[6] = { 9, 21, 11 ,1, 8, 4 };
    qsort_array(arr, 0, 5);

    for each (int num in arr)
    {
        printf("%d,", num);
    }
...
void qsort_array(int a[], int low, int high) {
    ...
    /*if the array has one element we return from function*/
    if (low >= high) return;
    
    int pivot = partition(a, low, high);
    qsort_array(a, low, pivot - 1);
    qsort_array(a, pivot + 1, high);

}


int partition(int arr[], int low, int high) {
    /*let the pivot be the last element of our array*/
    int pivot = high;

    /*i will only be moved if j has found an element less than the pivot,
        so we keep it outside the boundary of the array */
    int i = low - 1;
    /*since our pivot is the last element, we place j before the pivot*/
    int j = low;

    /*now we compare the elements with the pivot and swap them if they are less than the pivot*/
...
void swap(int arr[], int a, int b) {
    //printf("Swapping: %d and %d", arr[a], arr[b]);
    int temp = arr[a];
    arr[a] = arr[b];
    arr[b] = temp;
}
